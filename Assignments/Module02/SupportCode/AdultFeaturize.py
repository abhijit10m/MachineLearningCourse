...
class AdultFeaturize(object):
    ...
    def __init__(self):
        ...
        self.stddev = defaultdict(lambda: 1)
        self.min = defaultdict(lambda: 0)
        self.max = defaultdict(lambda: 1)
        self.featureSetCreated = False

    def _calculateMinAndMax(self, xRaw):
        for i in range(len(self.featureNames)):
            if self.useFeature[i] and self.isNumeric[i]:
                values = [ float(xRaw[j][i]) for j in range(len(xRaw)) ]
                self.min[i] = np.min(values)
                self.max[i] = np.max(values)

    def CreateFeatureSet(self, xRaw, yRaw, useCategoricalFeatures=True, useNumericFeatures=False, normalize=False):
    ...
    def _FeaturizeX(self, xRaw):
        ...
        for i in range(len(self.useFeature)):
            if self.useFeature[i]:
                if self.isNumeric[i]:
                    if self.useNormalization:
                        featureVector.append((float(xRaw[i]) - self.min[i])/(self.max[i] - self.min[i]))
                    else:
    ...
    def Featurize(self, xSetRaw):
        ...
        return np.array([ self._FeaturizeX(x) for x in xSetRaw ])
